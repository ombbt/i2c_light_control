...
#define bot_NUM_LEDS 120
#define top_NUM_LEDS 120
#define name_NUM_LEDS 60
#define clavier_NUM_LEDS 140

#define top_PIN 50
#define name_PIN 48
#define bot_PIN 46
#define clavier_PIN 44

// Define the array of leds
CRGB name_leds[name_NUM_LEDS];
CRGB top_leds[top_NUM_LEDS];
CRGB bot_leds[bot_NUM_LEDS];
CRGB clavier_leds[clavier_NUM_LEDS];

// Adafruit_PWMServoDriver pwm1 = Adafruit_PWMServoDriver(0x40);
// LED on pin 13
const int neon_merci = 29;
const int neon_sym = 31;
const int neon_GC = 33;
const int neon_TomB = 35;
const int neon_TomA = 37;
const int neon_CC = 39;
...
//                                        piezo                                     //
//////////////////////////////////////////////////////////////////////////////////////
int analogPin0 = A0; //  connected to analog pin 0                 // outside leads to ground and +5V
int A0_val = 0;  // variable to store the value read
int A0_max = 0;

int analogPin1 = A1; //  connected to analog pin 0                 // outside leads to ground and +5V
int A1_val = 0;  // variable to store the value read
int A1_max = 0;

int analogPin2 = A2; //  connected to analog pin 0                 // outside leads to ground and +5V
int A2_val = 0;  // variable to store the value read
int A2_max = 0;

int analogPin3 = A3; //  connected to analog pin 0                 // outside leads to ground and +5V
int A3_val = 0;  // variable to store the value read
int A3_max = 0;

int analogPin4 = A4; //  connected to analog pin 0                 // outside leads to ground and +5V
int A4_val = 0;  // variable to store the value read
int A4_max = 0;

int analogPin5 = A5; //  connected to analog pin 0                 // outside leads to ground and +5V
int A5_val = 0;  // variable to store the value read
int A5_max = 0;

int analogPin6 = A6; //  connected to analog pin 0                 // outside leads to ground and +5V
int A6_val = 0;  // variable to store the value read
int A6_max = 0;
...
void timeonefect( unsigned long finaltime){
  if(finaltime < timespend){
   ...
   if((finaltime - 100) > timespend){
    efectfin = 0;
  }
  if((finaltime - 100) < timespend and (finaltime - 10) > timespend){
    efectfin = 1;
  }
...
void top_piezobackgroudstrombo(int oncolor[], unsigned long ontime, unsigned long offtime, int count) {
  ...
  if(top_piezobackgroudstrombo_status == 1){
    ...
    fill_solid(top_leds, top_NUM_LEDS, CRGB(oncolor[0],oncolor[1],oncolor[2]));
...
void top_fixedcolor(int color[]){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color[0],color[1],color[2]));
}
...
void top_fadetofix2(int startcolor[], int endcolor[], unsigned long finaltime) {
  ...
  if(advance < 1000){
    ...
    fill_solid(top_leds, top_NUM_LEDS, CRGB(r,g,b));
    top_nothingtodo = 0;
  }
  if(advance >= 1000){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(endcolor[0],endcolor[1],endcolor[2]));
...
void top_strombo(int oncolor[], int offcolor[], unsigned long ontime, unsigned long offtime) {
  ...
  if(top_strombo_status == 0){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(offcolor[0],offcolor[1],offcolor[2]));
...
void top_somemulticolors(int color1[], int color2[], int color3[], int color4[], unsigned long time1, unsigned long time2, unsigned long time3, unsigned long time4, unsigned long donothingtime) {
  ...
  if(top_somemulticolors_status == 0){
    ...
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color1[0],color1[1],color1[2]));
  ...
  if(top_somemulticolors_status == 1){
    top_nothingtodo = 0;
    top_somemulticolors_active = 1;
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color2[0],color2[1],color2[2]));
  ...
  if(top_somemulticolors_status == 2){
    top_nothingtodo =0;
    top_somemulticolors_active = 1;
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color3[0],color3[1],color3[2]));
  ...
  if(top_somemulticolors_status == 3){
    top_nothingtodo =0;
    top_somemulticolors_active = 1;
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color4[0],color4[1],color4[2]));
...
unsigned long top_colors8_timmer;   
...
void top_colors8(int color1[], int color2[], int color3[], int color4[], int color5[], int color6[], int color7[], int color8[], unsigned long time1, unsigned long time2, unsigned long time3, unsigned long time4, unsigned long time5, unsigned long time6, unsigned long time7, unsigned long time8) {
  ...
  if(top_colors8_status == 4){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color5[0],color5[1],color5[2]));
  ...
  if(top_colors8_status == 5){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color6[0],color6[1],color6[2]));
  ...
  if(top_colors8_status == 6){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color7[0],color7[1],color7[2]));
  ...
  if(top_colors8_status == 7){
    fill_solid(top_leds, top_NUM_LEDS, CRGB(color8[0],color8[1],color8[2]));
...
int top_fade2colors_timeset;
unsigned long top_fade2colors_timmer1;
unsigned long top_fade2colors_timmer2;
void top_fade2colors(int startcolor[], int endcolor[], unsigned long finaltime, int offset) {
  int r;
  int g;
  int b;
  float advance;
  if(top_fade2colors_timeset == 0){
    top_fade2colors_timmer1 = millis() - offset;
    top_fade2colors_timeset = 1;
  }
  top_fade2colors_timmer2 = millis() - top_fade2colors_timmer1;
  advance = 2000*top_fade2colors_timmer2/finaltime;
  if(top_fade2colors_timmer2 < finaltime/2){
    r = startcolor[0]*(1000 - (advance))/1000 + endcolor[0]*(advance)/1000;
    g = startcolor[1]*(1000 - (advance))/1000 + endcolor[1]*(advance)/1000;
    b = startcolor[2]*(1000 - (advance))/1000 + endcolor[2]*(advance)/1000;
  ...
  if(finaltime/2 <= top_fade2colors_timmer2 and top_fade2colors_timmer2 < finaltime){
    r = endcolor[0]*(1000 - (advance - 1000))/1000 + startcolor[0]*(advance - 1000)/1000;
    g = endcolor[1]*(1000 - (advance - 1000))/1000 + startcolor[1]*(advance - 1000)/1000;
    b = endcolor[2]*(1000 - (advance - 1000))/1000 + startcolor[2]*(advance - 1000)/1000;

    fill_solid(top_leds, top_NUM_LEDS, CRGB(r,g,b));
  }
  if(top_fade2colors_timmer2 > (finaltime)){
     top_fade2colors_timmer1 = millis();
     top_fade2colors_timmer2 = 0;
      
  }     
    
}

int top_fade3colors_timeset;
unsigned long top_fade3colors_timmer1;
unsigned long top_fade3colors_timmer2;
void top_fade3colors(int color1[], int color2[], int color3[], unsigned long finaltime, int offset) {
  int r;
  int g;
  int b;
  float advance;
  if(top_fade3colors_timeset == 0){
    top_fade3colors_timmer1 = millis() - offset;
    top_fade3colors_timeset = 1;
  }
  top_fade3colors_timmer2 = millis() - top_fade3colors_timmer1;
  advance = 3000*top_fade3colors_timmer2/finaltime;
  if(top_fade3colors_timmer2 < finaltime/3){
    r = color1[0]*(1000 - (advance))/1000 + color2[0]*(advance)/1000;
    g = color1[1]*(1000 - (advance))/1000 + color2[1]*(advance)/1000;
    b = color1[2]*(1000 - (advance))/1000 + color2[2]*(advance)/1000;

    fill_solid(top_leds, top_NUM_LEDS, CRGB(r,g,b));
    }
    
  if(finaltime/3 <= top_fade3colors_timmer2 and top_fade3colors_timmer2 < finaltime*2/3){
    r = color2[0]*(1000 - (advance - 1000))/1000 + color3[0]*(advance - 1000)/1000;
    g = color2[1]*(1000 - (advance - 1000))/1000 + color3[1]*(advance - 1000)/1000;
    b = color2[2]*(1000 - (advance - 1000))/1000 + color3[2]*(advance - 1000)/1000;

    fill_solid(top_leds, top_NUM_LEDS, CRGB(r,g,b));
  }

  if(finaltime*2/3 <= top_fade3colors_timmer2 and top_fade3colors_timmer2 < finaltime){
    r = color3[0]*(1000 - (advance - 2000))/1000 + color1[0]*(advance - 2000)/1000;
    g = color3[1]*(1000 - (advance - 2000))/1000 + color1[1]*(advance - 2000)/1000;
    b = color3[2]*(1000 - (advance - 2000))/1000 + color1[2]*(advance - 2000)/1000;

    fill_solid(top_leds, top_NUM_LEDS, CRGB(r,g,b));
  }
  if(top_fade3colors_timmer2 > (finaltime)){
     top_fade3colors_timmer1 = millis();
     top_fade3colors_timmer2 = 0;
...
void bot_fade2colors(int startcolor[], int endcolor[], unsigned long finaltime, int offset) {
  int r;
  int g;
  int b;
  float advance;
  if(bot_fade2colors_timeset == 0){
    bot_fade2colors_timmer1 = millis() - offset;
    bot_fade2colors_timeset = 1;
  }
  bot_fade2colors_timmer2 = millis() - bot_fade2colors_timmer1;
  advance = 2000*bot_fade2colors_timmer2/finaltime;
  if(bot_fade2colors_timmer2 < finaltime/2){
    r = startcolor[0]*(1000 - (advance))/1000 + endcolor[0]*(advance)/1000;
    g = startcolor[1]*(1000 - (advance))/1000 + endcolor[1]*(advance)/1000;
    b = startcolor[2]*(1000 - (advance))/1000 + endcolor[2]*(advance)/1000;
  ...
  if(finaltime/2 <= bot_fade2colors_timmer2 and bot_fade2colors_timmer2 < finaltime){
    r = endcolor[0]*(1000 - (advance - 1000))/1000 + startcolor[0]*(advance - 1000)/1000;
    g = endcolor[1]*(1000 - (advance - 1000))/1000 + startcolor[1]*(advance - 1000)/1000;
    b = endcolor[2]*(1000 - (advance - 1000))/1000 + startcolor[2]*(advance - 1000)/1000;
  ...
  if(bot_fade2colors_timmer2 > (finaltime)){
     bot_fade2colors_timmer1 = millis();
     bot_fade2colors_timmer2 = 0;
...
int bot_fade3colors_timeset;
unsigned long bot_fade3colors_timmer1;
unsigned long bot_fade3colors_timmer2;
void bot_fade3colors(int color1[], int color2[], int color3[], unsigned long finaltime, int offset) {
  int r;
  int g;
  int b;
  float advance;
  if(bot_fade3colors_timeset == 0){
    bot_fade3colors_timmer1 = millis() - offset;
    bot_fade3colors_timeset = 1;
  }
  bot_fade3colors_timmer2 = millis() - bot_fade3colors_timmer1;
  advance = 3000*bot_fade3colors_timmer2/finaltime;
  if(bot_fade3colors_timmer2 < finaltime/3){
    r = color1[0]*(1000 - (advance))/1000 + color2[0]*(advance)/1000;
    g = color1[1]*(1000 - (advance))/1000 + color2[1]*(advance)/1000;
    b = color1[2]*(1000 - (advance))/1000 + color2[2]*(advance)/1000;
  ...
  if(finaltime/3 <= bot_fade3colors_timmer2 and bot_fade3colors_timmer2 < finaltime*2/3){
    r = color2[0]*(1000 - (advance - 1000))/1000 + color3[0]*(advance - 1000)/1000;
    g = color2[1]*(1000 - (advance - 1000))/1000 + color3[1]*(advance - 1000)/1000;
    b = color2[2]*(1000 - (advance - 1000))/1000 + color3[2]*(advance - 1000)/1000;
  ...
  if(finaltime*2/3 <= bot_fade3colors_timmer2 and bot_fade3colors_timmer2 < finaltime){
    r = color3[0]*(1000 - (advance - 2000))/1000 + color1[0]*(advance - 2000)/1000;
    g = color3[1]*(1000 - (advance - 2000))/1000 + color1[1]*(advance - 2000)/1000;
    b = color3[2]*(1000 - (advance - 2000))/1000 + color1[2]*(advance - 2000)/1000;
  ...
  if(bot_fade3colors_timmer2 > (finaltime)){
     bot_fade3colors_timmer1 = millis();
     bot_fade3colors_timmer2 = 0;
...
unsigned long disco_fade2colors_timmer2;
void disco_fade2colors(int startcolor[], int endcolor[], unsigned long finaltime, int offset) {
  int r;
  int g;
  int b;
  float advance;
  if(disco_fade2colors_timeset == 0){
    disco_fade2colors_timmer1 = millis() - offset;
    disco_fade2colors_timeset = 1;
  }
  disco_fade2colors_timmer2 = millis() - disco_fade2colors_timmer1;
  advance = 2000*disco_fade2colors_timmer2/finaltime;
  if(disco_fade2colors_timmer2 < finaltime/2){
    r = startcolor[0]*(1000 - (advance))/1000 + endcolor[0]*(advance)/1000;
    g = startcolor[1]*(1000 - (advance))/1000 + endcolor[1]*(advance)/1000;
    b = startcolor[2]*(1000 - (advance))/1000 + endcolor[2]*(advance)/1000;
  ...
  if(finaltime/2 <= disco_fade2colors_timmer2 and disco_fade2colors_timmer2 < finaltime){
    r = endcolor[0]*(1000 - (advance - 1000))/1000 + startcolor[0]*(advance - 1000)/1000;
    g = endcolor[1]*(1000 - (advance - 1000))/1000 + startcolor[1]*(advance - 1000)/1000;
    b = endcolor[2]*(1000 - (advance - 1000))/1000 + startcolor[2]*(advance - 1000)/1000;
  ...
  if(disco_fade2colors_timmer2 > (finaltime)){
     disco_fade2colors_timmer1 = millis();
     disco_fade2colors_timmer2 = 0;
...
unsigned long disco_fade3colors_timmer2;
void disco_fade3colors(int color1[], int color2[], int color3[], unsigned long finaltime, int offset) {
  int r;
  int g;
  int b;
  float advance;
  if(disco_fade3colors_timeset == 0){
    disco_fade3colors_timmer1 = millis() - offset;
    disco_fade3colors_timeset = 1;
  }
  disco_fade3colors_timmer2 = millis() - disco_fade3colors_timmer1;
  advance = 3000*disco_fade3colors_timmer2/finaltime;
  if(disco_fade3colors_timmer2 < finaltime/3){
    r = color1[0]*(1000 - (advance))/1000 + color2[0]*(advance)/1000;
    g = color1[1]*(1000 - (advance))/1000 + color2[1]*(advance)/1000;
    b = color1[2]*(1000 - (advance))/1000 + color2[2]*(advance)/1000;
  ...
  if(finaltime/3 <= disco_fade3colors_timmer2 and disco_fade3colors_timmer2 < finaltime*2/3){
    r = color2[0]*(1000 - (advance - 1000))/1000 + color3[0]*(advance - 1000)/1000;
    g = color2[1]*(1000 - (advance - 1000))/1000 + color3[1]*(advance - 1000)/1000;
    b = color2[2]*(1000 - (advance - 1000))/1000 + color3[2]*(advance - 1000)/1000;
  ...
  if(finaltime*2/3 <= disco_fade3colors_timmer2 and disco_fade3colors_timmer2 < finaltime){
    r = color3[0]*(1000 - (advance - 2000))/1000 + color1[0]*(advance - 2000)/1000;
    g = color3[1]*(1000 - (advance - 2000))/1000 + color1[1]*(advance - 2000)/1000;
    b = color3[2]*(1000 - (advance - 2000))/1000 + color1[2]*(advance - 2000)/1000;
  ...
  if(disco_fade3colors_timmer2 > (finaltime)){
     disco_fade3colors_timmer1 = millis();
     disco_fade3colors_timmer2 = 0;
...
void name_strombo(int oncolor[], int offcolor[], unsigned long ontime, unsigned long offtime) {  
  ...
  if(name_strombo_status == 1){
    fill_solid(name_leds, name_NUM_LEDS, CRGB(oncolor[0], oncolor[1], oncolor[2]));
  ...
  if(name_strombo_status == 0){
    fill_solid(name_leds, name_NUM_LEDS, CRGB(offcolor[0], offcolor[1], offcolor[2]));
...
void name_somemulticolors(int color1[], int color2[], int color3[], int color4[], unsigned long time1, unsigned long time2, unsigned long time3, unsigned long time4, unsigned long donothingtime) {
  ...
  if(name_somemulticolors_status == 0){
    ...
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color1[0], color1[1], color1[2]));
  ...
  if(name_somemulticolors_status == 1){
    name_somemulticolors_active = 1;
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color2[0], color2[1], color2[2]));
  ...
  if(name_somemulticolors_status == 2){
    name_somemulticolors_active = 1;
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color3[0], color3[1], color3[2]));
  ...
  if(name_somemulticolors_status == 3){
    name_somemulticolors_active = 1;
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color4[0], color4[1], color4[2]));
...
void name_colors8(int color1[], int color2[], int color3[], int color4[], int color5[], int color6[], int color7[], int color8[], unsigned long time1, unsigned long time2, unsigned long time3, unsigned long time4, unsigned long time5, unsigned long time6, unsigned long time7, unsigned long time8) {
  ...
  if(name_colors8_status == 4){
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color5[0], color5[1], color5[2]));
  ...
  if(name_colors8_status == 5){
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color6[0], color6[1], color6[2]));
  ...
  if(name_colors8_status == 6){
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color7[0], color7[1], color7[2]));
  ...
  if(name_colors8_status == 7){
    fill_solid(name_leds, name_NUM_LEDS, CRGB(color8[0],color8[1],color8[2]));
...
void name_fadesiren(int startcolor[], int endcolor[], unsigned long finaltime) {
  ...
  if(name_fadesiren_status == 1){
    ...
    g = startcolor[1]*(1000 - (advance))/1000 + endcolor[1]*(advance)/1000;
    b = startcolor[2]*(1000 - (advance))/1000 + endcolor[2]*(advance)/1000;
    fill_solid(name_leds, name_NUM_LEDS, CRGB(r,g,b));
...
void name_fade1(int startcolor[], int endcolor[], unsigned long totaltime, int offsetlist[]) {
  ...
  if(T1_timmer2 < finaltime/2){
    ...
    name_leds[0] = CRGB(T1r,T1g,T1b);
    name_leds[1] = CRGB(T1r,T1g,T1b);
    }
  ...
  if(H1_timmer2 < finaltime/2){
    ...
    name_leds[2] = CRGB(H1r,H1g,H1b);
    name_leds[3] = CRGB(H1r,H1g,H1b);
    }
  ...
  if(E1_timmer2 < finaltime/2){
    ...
    name_leds[4] = CRGB(E1r,E1g,E1b);
    name_leds[5] = CRGB(E1r,E1g,E1b);
    }
  ...
  if(O2_timmer2 < finaltime/2){
    ...
    name_leds[6] = CRGB(O2r,O2g,O2b);
    name_leds[7] = CRGB(O2r,O2g,O2b);
    }
  ...
  if(N2_timmer2 < finaltime/2){
    ...
    name_leds[8] = CRGB(N2r,N2g,N2b);
    name_leds[9] = CRGB(N2r,N2g,N2b);
    }
  ...
  if(E2_timmer2 < finaltime/2){
    ...
    name_leds[10] = CRGB(E2r,E2g,E2b);
    name_leds[11] = CRGB(E2r,E2g,E2b);
    }
  ...
  if(M3_timmer2 < finaltime/2){
    ...
    name_leds[12] = CRGB(M3r,M3g,M3b);
    name_leds[13] = CRGB(M3r,M3g,M3b);
    }
  ...
  if(A3_timmer2 < finaltime/2){
    ...
    name_leds[14] = CRGB(A3r,A3g,A3b);
    name_leds[15] = CRGB(A3r,A3g,A3b);
    }
  ...
  if(N3_timmer2 < finaltime/2){
    ...
    name_leds[16] = CRGB(N3r,N3g,N3b);
    name_leds[17] = CRGB(N3r,N3g,N3b);
    }
  ...
  if(B4_timmer2 < finaltime/2){
    ...
    name_leds[18] = CRGB(B4r,B4g,B4b);
    name_leds[19] = CRGB(B4r,B4g,B4b);
    }
  ...
  if(I4_timmer2 < finaltime/2){
    ...
    name_leds[20] = CRGB(I4r,I4g,I4b);
    name_leds[21] = CRGB(I4r,I4g,I4b);
    }
  ...
  if(G4_timmer2 < finaltime/2){
    ...
    name_leds[22] = CRGB(G4r,G4g,G4b);
    name_leds[23] = CRGB(G4r,G4g,G4b);
    }
  ...
  if(B5_timmer2 < finaltime/2){
    ...
    name_leds[24] = CRGB(B5r,B5g,B5b);
    name_leds[25] = CRGB(B5r,B5g,B5b);
    }
  ...
  if(A5_timmer2 < finaltime/2){
    ...
    name_leds[26] = CRGB(A5r,A5g,A5b);
    name_leds[27] = CRGB(A5r,A5g,A5b);
    }
  ...
  if(N5_timmer2 < finaltime/2){
    ...
    name_leds[28] = CRGB(N5r,N5g,N5b);
    name_leds[29] = CRGB(N5r,N5g,N5b);
    }
  ...
  if(D5_timmer2 < finaltime/2){
    ...
    name_leds[30] = CRGB(D5r,D5g,D5b);
    name_leds[31] = CRGB(D5r,D5g,D5b);
    }
  ...
  if(T6_timmer2 < finaltime/2){
    ...
    name_leds[32] = CRGB(T6r,T6g,T6b);
    name_leds[33] = CRGB(T6r,T6g,T6b);
    }
  ...
  if(H6_timmer2 < finaltime/2){
    ...
    name_leds[34] = CRGB(H6r,H6g,H6b);
    name_leds[35] = CRGB(H6r,H6g,H6b);
    }
  ...
  if(E6_timmer2 < finaltime/2){
    ...
    name_leds[36] = CRGB(E6r,E6g,E6b);
    name_leds[37] = CRGB(E6r,E6g,E6b);
    }
  ...
  if(O6_timmer2 < finaltime/2){
    ...
    name_leds[38] = CRGB(O6r,O6g,O6b);
    name_leds[39] = CRGB(O6r,O6g,O6b);
    }
  ...
  if(R6_timmer2 < finaltime/2){
    ...
    name_leds[40] = CRGB(R6r,R6g,R6b);
    name_leds[41] = CRGB(R6r,R6g,R6b);
    }
  ...
  if(Y6_timmer2 < finaltime/2){
    ...
    name_leds[42] = CRGB(Y6r,Y6g,Y6b);
    name_leds[43] = CRGB(Y6r,Y6g,Y6b);
    }
...
name_leds[0] = CRGB(color1[0],color1[1],color1[2]);
name_leds[1] = CRGB(color1[0],color1[1],color1[2]);

name_leds[2] = CRGB(color2[0],color2[1],color2[2]);
name_leds[3] = CRGB(color2[0],color2[1],color2[2]);

name_leds[4] = CRGB(color3[0],color3[1],color3[2]);
name_leds[5] = CRGB(color3[0],color3[1],color3[2]);

name_leds[6] = CRGB(color4[0],color4[1],color4[2]);
name_leds[7] = CRGB(color4[0],color4[1],color4[2]);

name_leds[8] = CRGB(color5[0],color5[1],color5[2]);
name_leds[9] = CRGB(color5[0],color5[1],color5[2]);

name_leds[10] = CRGB(color6[0],color6[1],color6[2]);
name_leds[11] = CRGB(color6[0],color6[1],color6[2]);

name_leds[12] = CRGB(color7[0],color7[1],color7[2]);
name_leds[13] = CRGB(color7[0],color7[1],color7[2]);

name_leds[14] = CRGB(color8[0],color8[1],color8[2]);
name_leds[15] = CRGB(color8[0],color8[1],color8[2]);

name_leds[16] = CRGB(color9[0],color9[1],color9[2]);
name_leds[17] = CRGB(color9[0],color9[1],color9[2]);

name_leds[18] = CRGB(color10[0],color10[1],color10[2]);
name_leds[19] = CRGB(color10[0],color10[1],color10[2]);

name_leds[20] = CRGB(color11[0],color11[1],color11[2]);
name_leds[21] = CRGB(color11[0],color11[1],color11[2]);

name_leds[22] = CRGB(color12[0],color12[1],color12[2]);
name_leds[23] = CRGB(color12[0],color12[1],color12[2]);

name_leds[24] = CRGB(color13[0],color13[1],color13[2]);
name_leds[25] = CRGB(color13[0],color13[1],color13[2]);

name_leds[26] = CRGB(color14[0],color14[1],color14[2]);
name_leds[27] = CRGB(color14[0],color14[1],color14[2]);

name_leds[28] = CRGB(color15[0],color15[1],color15[2]);
name_leds[29] = CRGB(color15[0],color15[1],color15[2]);

name_leds[30] = CRGB(color16[0],color16[1],color16[2]);
name_leds[31] = CRGB(color16[0],color16[1],color16[2]);

name_leds[32] = CRGB(color17[0],color17[1],color17[2]);
name_leds[33] = CRGB(color17[0],color17[1],color17[2]);

name_leds[34] = CRGB(color18[0],color18[1],color18[2]);
name_leds[35] = CRGB(color18[0],color18[1],color18[2]);

name_leds[36] = CRGB(color19[0],color19[1],color19[2]);
name_leds[37] = CRGB(color19[0],color19[1],color19[2]);

name_leds[38] = CRGB(color20[0],color20[1],color20[2]);
name_leds[39] = CRGB(color20[0],color20[1],color20[2]);

name_leds[40] = CRGB(color21[0],color21[1],color21[2]);
name_leds[41] = CRGB(color21[0],color21[1],color21[2]);

name_leds[42] = CRGB(color22[0],color22[1],color22[2]);
name_leds[43] = CRGB(color22[0],color22[1],color22[2]);
}

void name_THEfixedcolors(int Tcolor[], int Hcolor[], int Ecolor[]) {
  
    name_leds[0] = CRGB(Tcolor[0],Tcolor[1],Tcolor[2]);
    name_leds[1] = CRGB(Tcolor[0],Tcolor[1],Tcolor[2]);

    name_leds[2] = CRGB(Hcolor[0],Hcolor[1],Hcolor[2]);
    name_leds[3] = CRGB(Hcolor[0],Hcolor[1],Hcolor[2]);

    name_leds[4] = CRGB(Ecolor[0],Ecolor[1],Ecolor[2]);
    name_leds[5] = CRGB(Ecolor[0],Ecolor[1],Ecolor[2]);
    
}

void name_ONEfixedcolors(int Ocolor[], int Ncolor[], int Ecolor[]) {
  
    name_leds[6] = CRGB(Ocolor[0],Ocolor[1],Ocolor[2]);
    name_leds[7] = CRGB(Ocolor[0],Ocolor[1],Ocolor[2]);

    name_leds[8] = CRGB(Ncolor[0],Ncolor[1],Ncolor[2]);
    name_leds[9] = CRGB(Ncolor[0],Ncolor[1],Ncolor[2]);

    name_leds[10] = CRGB(Ecolor[0],Ecolor[1],Ecolor[2]);
    name_leds[11] = CRGB(Ecolor[0],Ecolor[1],Ecolor[2]);
    
}

void name_MANfixedcolors(int Mcolor[], int Acolor[], int Ncolor[]) {
  
    name_leds[12] = CRGB(Mcolor[0],Mcolor[1],Mcolor[2]);
    name_leds[13] = CRGB(Mcolor[0],Mcolor[1],Mcolor[2]);
    
    name_leds[14] = CRGB(Acolor[0],Acolor[1],Acolor[2]);
    name_leds[15] = CRGB(Acolor[0],Acolor[1],Acolor[2]);
    
    name_leds[16] = CRGB(Ncolor[0],Ncolor[1],Ncolor[2]);
    name_leds[17] = CRGB(Ncolor[0],Ncolor[1],Ncolor[2]);
    
}

void name_BIGfixedcolors(int Bcolor[], int Icolor[], int Gcolor[]) {
  
    name_leds[18] = CRGB(Bcolor[0],Bcolor[1],Bcolor[2]);
    name_leds[19] = CRGB(Bcolor[0],Bcolor[1],Bcolor[2]);
    
    name_leds[20] = CRGB(Icolor[0],Icolor[1],Icolor[2]);
    name_leds[21] = CRGB(Icolor[0],Icolor[1],Icolor[2]);
    
    name_leds[22] = CRGB(Gcolor[0],Gcolor[1],Gcolor[2]);
    name_leds[23] = CRGB(Gcolor[0],Gcolor[1],Gcolor[2]);
}

void name_BANDfixedcolors(int Bcolor[], int Acolor[], int Ncolor[], int Dcolor[]) {
  
    name_leds[24] = CRGB(Bcolor[0],Bcolor[1],Bcolor[2]);
    name_leds[25] = CRGB(Bcolor[0],Bcolor[1],Bcolor[2]);
    
    name_leds[26] = CRGB(Acolor[0],Acolor[1],Acolor[2]);
    name_leds[27] = CRGB(Acolor[0],Acolor[1],Acolor[2]);
    
    name_leds[28] = CRGB(Ncolor[0],Ncolor[1],Ncolor[2]);
    name_leds[29] = CRGB(Ncolor[0],Ncolor[1],Ncolor[2]);

    name_leds[30] = CRGB(Dcolor[0],Dcolor[1],Dcolor[2]);
    name_leds[31] = CRGB(Dcolor[0],Dcolor[1],Dcolor[2]);
     
}

void name_THEORYfixedcolors(int Tcolor[], int Hcolor[], int Ecolor[], int Ocolor[], int Rcolor[], int Ycolor[]) {
  
    name_leds[32] = CRGB(Tcolor[0],Tcolor[1],Tcolor[2]);
    name_leds[33] = CRGB(Tcolor[0],Tcolor[1],Tcolor[2]);

    name_leds[34] = CRGB(Hcolor[0],Hcolor[1],Hcolor[2]);
    name_leds[35] = CRGB(Hcolor[0],Hcolor[1],Hcolor[2]);

    name_leds[36] = CRGB(Ecolor[0],Ecolor[1],Ecolor[2]);
    name_leds[37] = CRGB(Ecolor[0],Ecolor[1],Ecolor[2]);

    name_leds[38] = CRGB(Ocolor[0],Ocolor[1],Ocolor[2]);
    name_leds[39] = CRGB(Ocolor[0],Ocolor[1],Ocolor[2]);

    name_leds[40] = CRGB(Rcolor[0],Rcolor[1],Rcolor[2]);
    name_leds[41] = CRGB(Rcolor[0],Rcolor[1],Rcolor[2]);

    name_leds[42] = CRGB(Ycolor[0],Ycolor[1],Ycolor[2]);
    name_leds[43] = CRGB(Ycolor[0],Ycolor[1],Ycolor[2]);
...
///////////////////////////////////////////////////////
void clavier_test(){
  clavier_leds[130] = CRGB( 0,255,125);
  clavier_leds[120] = CRGB( 255,125,0);          
...
//                fonctions init                  ///
/////////////////////////////////////////////////////
void name_timmersinit(){
name_strombo_timeset  = 0;
name_strombo_status = 0;
name_backgroundstrombo_timeset = 0;
name_backgroundstrombo_status = 0;
name_someflasch_timeset = 0;
name_someflasch_status = 0;
name_someblack_timeset = 0;
name_someblack_status = 0;
name_somemulticolors_timeset = 0;
name_somemulticolors_status = 0;
name_colors8_timeset = 0;
name_colors8_status = 0;
name_lightoffbycenter_timeset = 0;
name_lightoffbycenter_status = 0;
name_lightoffbycenter_loopcount = 0;
name_fadesiren_status = 0;
name_fadesiren_timeset = 0;
name_fade1_init = 0;
name_fade3_init = 0;
name_THEblink_timeset = 0;
name_THEblink_status = 0;
name_THEblink_loopcount = 0;
name_ONEblink_timeset = 0;
name_ONEblink_status = 0;
name_ONEblink_loopcount = 0;
name_MANblink_timeset = 0;
name_MANblink_status = 0;
name_MANblink_loopcount = 0;
name_BIGblink_timeset = 0;
name_BIGblink_status = 0;
name_BIGblink_loopcount = 0;
name_BANDblink_timeset = 0;
name_BANDblink_status = 0;
name_BANDblink_loopcount = 0;
name_THEORYblink_timeset = 0;
name_THEORYblink_status = 0;
name_THEORYblink_loopcount = 0;
name_THEchenillar1_timeset = 0;
name_THEchenillar1_status = 0;
name_THEchenillar1_loopcount = 0;
name_THEchenillar2_timeset = 0;
name_THEchenillar2_status = 0;
name_THEchenillar2_loopcount = 0;
name_THEchenillar3_timeset = 0;
name_THEchenillar3_status = 0;
name_THEchenillar3_loopcount = 0;
name_ONEchenillar1_timeset = 0;
name_ONEchenillar1_status = 0;
name_ONEchenillar1_loopcount = 0;
name_ONEchenillar2_timeset = 0;
name_ONEchenillar2_status = 0;
name_ONEchenillar2_loopcount = 0;
name_ONEchenillar3_timeset = 0;
name_ONEchenillar3_status = 0;
name_ONEchenillar3_loopcount = 0;
name_MANchenillar1_timeset = 0;
name_MANchenillar1_status = 0;
name_MANchenillar1_loopcount = 0;
name_MANchenillar2_timeset = 0;
name_MANchenillar2_status = 0;
name_MANchenillar2_loopcount = 0;
name_MANchenillar3_timeset = 0;
name_MANchenillar3_status = 0;
name_MANchenillar3_loopcount = 0;
name_BIGchenillar1_timeset = 0;
name_BIGchenillar1_status = 0;
name_BIGchenillar1_loopcount = 0;
name_BIGchenillar2_timeset = 0;
name_BIGchenillar2_status = 0;
name_BIGchenillar2_loopcount = 0;
name_BIGchenillar3_timeset = 0;
name_BIGchenillar3_status = 0;
name_BIGchenillar3_loopcount = 0;
name_BANDchenillar1_timeset = 0;
name_BANDchenillar1_status = 0;
name_BANDchenillar1_loopcount = 0;
name_BANDchenillar2_timeset = 0;
name_BANDchenillar2_status = 0;
name_BANDchenillar2_loopcount = 0;
name_BANDchenillar3_timeset = 0;
name_BANDchenillar3_status = 0;
name_BANDchenillar3_loopcount = 0;
name_THEORYchenillar1_timeset = 0;
name_THEORYchenillar1_status = 0;
name_THEORYchenillar1_loopcount = 0;
name_THEORYchenillar2_timeset = 0;
name_THEORYchenillar2_status = 0;
name_THEORYchenillar2_loopcount = 0;
name_THEORYchenillar3_timeset = 0;
name_THEORYchenillar3_status = 0;
name_THEORYchenillar3_loopcount = 0;
name_THEORYchenillar4_timeset = 0;
name_THEORYchenillar4_status = 0;
name_THEORYchenillar4_loopcount = 0;
name_THEORYchenillar5_timeset = 0;
name_THEORYchenillar5_status = 0;
name_THEORYchenillar5_loopcount = 0;
name_chenillarblanc_timeset = 0;
name_chenillarblanc_status = 0;
name_chenillarblanc_loopcount = 0;
name_enchainement1_timeset = 0;
name_enchainement1_status = 0;
name_chenillarcolor1_timeset = 0;
name_chenillarcolor1_status = 0;
name_chenillarcolor1_loopcount = 0;
name_chenillar3color_timeset = 0;
name_chenillar3color_status = 0;
name_chenillar3color_loopcount = 0;
name_chenillartype2et3color_timeset = 0;
name_chenillartype2et3color_status = 0;
name_chenillartype2et3color_loopcount = 0;
name_chenillartype3et3color_timeset = 0;
name_chenillartype3et3color_status = 0;
name_chenillartype3et3color_loopcount = 0;
name_automaticchenillar_timeset= 0;
name_automaticchenillar_status = 0;
name_automaticchenillar_loopcount = 0;
name_automaticenchainement1_timeset = 0;
name_automaticenchainement1_status = 0;
name_automaticenchainement1_loopcount = 0;
}
void top_timmersinit(){
  ...
  top_someblack_timeset = 0;
  top_somemulticolors_timeset = 0;
  top_somemulticolors_status = 0;
  top_colors3_timeset = 0;
  top_colors3_status = 0;
  top_colors8_timeset = 0;
  top_colors8_status = 0;
  top_fadesiren_status = 0;
  top_fadesiren_timeset = 0;
  top_fade2colors_timeset = 0;
  top_fade3colors_timeset = 0;
}

void bot_timmersinit(){
  bot_fadetofix2_timeset = 0;
  bot_strombo_timeset = 0;
  bot_backgroundstrombo_timeset = 0;
  bot_someflasch_timeset = 0;
  bot_someblack_timeset = 0;
  bot_somemulticolors_timeset = 0;
  bot_somemulticolors_status = 0;
  bot_colors3_timeset = 0;
  bot_colors3_status = 0;
  bot_colors8_timeset = 0;
  bot_colors8_status = 0;
  bot_fadesiren_status = 0;
  bot_fadesiren_timeset = 0;
  bot_fade2colors_timeset = 0;
  bot_fade3colors_timeset = 0;
}

void disco_timmersinit(){
  disco_fadetofix2_timeset = 0;
  disco_strombo_timeset = 0;
  disco_backgroundstrombo_timeset = 0;
  disco_someflasch_timeset = 0;
  disco_someblack_timeset = 0;
  disco_somemulticolors_timeset = 0;
  disco_somemulticolors_status = 0;
  disco_colors3_timeset = 0;
  disco_colors3_status = 0;
  disco_colors8_timeset = 0;
  disco_colors8_status = 0;
  disco_fadesiren_status = 0;
  disco_fadesiren_timeset = 0;
  disco_fade2colors_timeset = 0;
  disco_fade3colors_timeset = 0;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void all_lightoff() {
    fill_solid(name_leds, name_NUM_LEDS, CRGB(0,0,0));
...
void mode2() {
  ...
  if((efecton == 0 or disco_nothingtodo == 1) and disco_someflasch_activate == 1){
    disco_someflasch(blanc, 20, 5000);
  }
  if((efecton == 0 or disco_nothingtodo == 1) and disco_somemulticolors_activate == 1 and disco_someflasch_active == 0){
    disco_somemulticolors(bleu, vert, blanc, rose, 100, 100, 100, 100, 4000);
  }
  if((efecton == 0 or disco_nothingtodo == 1) and disco_someblack_activate == 1 and disco_somemulticolors_active == 0 and disco_someflasch_active == 0){
    disco_someblack(noir, 400, 3600);
  }
  ...
  if((disco_someflasch_activate == 0 and disco_someblack_activate == 0 and disco_somemulticolors_activate == 0 and efecton == 0) or disco_nothingtodo == 1){
    switch(program){//de 128 a 143 (de 0x80 a 0x8F)
      
      case 128:
        disco_fadesiren(bleu, vert, 8000);
        disco_someflasch_activate = 1;
        disco_someblack_activate = 1;
        disco_somemulticolors_activate = 1;
...
if(efecton == 1){
  ...
  if((top_someflasch_activate == 0 and top_someblack_activate == 0 and top_somemulticolors_activate == 0 and efecton == 0) or top_nothingtodo == 1){
    switch(program){//de 128 a 143 (de 0x80 a 0x8F)
      ...
      case 128:
        ...
        top_fade2colors(bleu, noir, 1200, 600);
        top_someflasch_activate = 0;
        top_someblack_activate = 0;
        top_somemulticolors_activate = 0;
        namestatus = 1;
  ...
  if((bot_someflasch_activate == 0 and bot_someblack_activate == 0 and bot_somemulticolors_activate == 0 and efecton == 0) or bot_nothingtodo == 1){
    ...
    switch(program){//de 128 a 143 (de 0x80 a 0x8F)
      ...
      case 128:
        bot_fade2colors(bleu, noir, 1200, 0);
        bot_someflasch_activate = 0;
        bot_someblack_activate = 0;
        bot_somemulticolors_activate = 0;
...
if(efecton == 1){
  ...
  if((top_someflasch_activate == 0 and top_someblack_activate == 0 and top_somemulticolors_activate == 0 and efecton == 0) or top_nothingtodo == 1){
    switch(program){//de 128 a 143 (de 0x80 a 0x8F)
      ...
      case 129: //couplet
        top_colors8(ciel, noir, blanc, noir, rose, noir, blanc, noir, 3000, 300, 20, 300, 3000, 300, 200, 300);
        top_someflasch_activate = 0;
        top_someblack_activate = 0;
        top_somemulticolors_activate = 0;
  ...
  if((bot_someflasch_activate == 0 and bot_someblack_activate == 0 and bot_somemulticolors_activate == 0 and efecton == 0) or bot_nothingtodo == 1){
    ...
    switch(program){//de 128 a 143 (de 0x80 a 0x8F)
      ...
      case 129:
        top_colors8(noir, blanc, noir, rose, noir, blanc, noir, ciel, 300, 20, 300, 3000, 300, 200, 300, 3000);
        bot_someflasch_activate = 0;
        bot_someblack_activate = 0;
        bot_somemulticolors_activate = 0;
  ...
  if((disco_someflasch_activate == 0 and disco_someblack_activate == 0 and disco_somemulticolors_activate == 0 and efecton == 0) or disco_nothingtodo == 1){
    switch(program){//de 128 a 143 (de 0x80 a 0x8F)
      ...
      case 128:
        disco_colors3(rose, ciel, jaune, 6000, 6000, 6000);
      ...
      case 129:
        disco_fade3colors(blanc, jaune, rose, 5000, 300);
        disco_someflasch_activate = 0;
        disco_someblack_activate = 1;
        disco_somemulticolors_activate = 0;
      ...
      case 130:
        disco_colors3(rose, blanc, ciel, 1200, 1200, 1200);
        disco_someflasch_activate = 0;
        disco_someblack_activate = 0;
        disco_somemulticolors_activate =0;
  ...
  switch(namestatus){  
      ...
      case 1:
        ...
        name_lightoffbycenter(ciel, rose, jaune, ciel, rose, jaune, blanc, ciel, rose, jaune, ciel, rose, jaune, ciel, rose, jaune, ciel, rose, jaune, ciel, rose, jaune, 200);
       if(name_lightoffbycenter_loopcount >= 1){
         name_fixedcolors(noir);
...
//if you feel it
void mode5() {
  top_fade2colors(bleu, vert, 2000, 500);
  bot_fade2colors(bleu, vert, 2000, 1500);
  disco_fade3colors(darkrouge, darkorange, darkvert, 8000, 300);
...
void setup() {
  ...
  pinMode(neon_merci, OUTPUT);
  pinMode(neon_sym, OUTPUT);
  pinMode(neon_GC, OUTPUT);
  pinMode(neon_TomB, OUTPUT);
  pinMode(neon_TomA, OUTPUT);
  pinMode(neon_CC, OUTPUT);
  ...
  FastLED.addLeds<NEOPIXEL, name_PIN>(name_leds, name_NUM_LEDS);
  FastLED.addLeds<NEOPIXEL, top_PIN>(top_leds, top_NUM_LEDS);
  FastLED.addLeds<NEOPIXEL, bot_PIN>(bot_leds, bot_NUM_LEDS);
  FastLED.addLeds<NEOPIXEL, clavier_PIN>(clavier_leds, clavier_NUM_LEDS);
  ...
  if(128 <= x and x < 144)
   ...
   if(144 <= x and x < 160)
    ...
    if(lightonoff == true){
      ...
      ///////////////////// lecture piezo //////////////////////
      A0_val = map(analogRead(analogPin0), 0, 1023, 60, 127);
